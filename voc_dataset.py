...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.top_n is not None:
            img_list = self.roi_data['images']

            img_index = np.where(img_list[0]==findex)[0][0]
            raw_box = self.roi_data['boxes'][0][img_index][:self.top_n]
            proposals = np.empty([raw_box.shape[0],raw_box.shape[1]],dtype=np.float64)
            box_idx = np.arange(self.top_n)
            #proposals[:,0] = box_idx
            proposals[:,0] = raw_box[:,1]/width   #xmin
            proposals[:,1] = raw_box[:,0]/height  #ymin
            proposals[:,2] = raw_box[:,3]/width   #xmax
            proposals[:,3] = raw_box[:,2]/height  #ymax
            ret['rois'] = proposals
